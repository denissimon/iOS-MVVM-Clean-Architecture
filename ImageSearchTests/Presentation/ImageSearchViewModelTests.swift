...
class ImageSearchViewModelTests: XCTestCase {
    ...
    func testSearchImage_whenSearchQueryIsValid_andWhenResultIsSuccess() async throws {
        ...
        XCTAssertTrue((imageSearchViewModel.data.value.data[0]._searchResults as! [Image]).contains(ImageSearchViewModelTests.testImageStub))
        if let expectedImageData = UIImage(systemName: "heart.fill")?.pngData() {
            XCTAssertEqual((imageSearchViewModel.data.value.data[0]._searchResults as! [Image])[0].thumbnail?.uiImage?.pngData(), Supportive.toUIImage(from: expectedImageData)?.pngData())
        }
    ...
    func testSearchImage_whenSearchIsRunTwice() async throws {
        ...
        XCTAssertTrue((imageSearchViewModel.data.value.data[0]._searchResults as! [Image]).contains(ImageSearchViewModelTests.testImageStub))
        XCTAssertTrue((imageSearchViewModel.data.value.data[1]._searchResults as! [Image]).contains(ImageSearchViewModelTests.testImageStub))
    ...
    func testUpdateSection() async throws {
        ...
        for image in imageSearchViewModel.data.value.data[3]._searchResults {
            XCTAssertNil(image.thumbnail)
        }
        for image in imageSearchViewModel.data.value.data[4]._searchResults {
            XCTAssertNil(image.thumbnail)
        }
